...
const StoreListPage = () => {
  ...
  const handleActionItem = async (name: string, value: any) => {
    ...
    if (name === 'edit') {
      const res = await detailFetchData(value?.id);
      handleOpenModal('edit', res as Store);
    }
  };

  const handleOpenModal = (mode: 'add' | 'edit' | 'detail', value: Store | null) => {
  ...
  const handleConfirmModal = async (mode: 'add' | 'edit' | 'detail', data: Store) => {
    ...
    if (mode === 'edit' && store) {
      const res = await editMutate(store.id, data);
